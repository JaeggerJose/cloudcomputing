...
class P2PNode:
    def __init__(self, port, peers):
        ...
        self.sock.bind(('172.17.0.21', self.port)) #這是本節點的 IP
        self.node = '172.17.0.21'
        self.shaList = []
        self.blockList = []
    ...
    def cmdParser(self, argv, addr):
        ...
        if argv[0] == "checkMoney":
            ...
            if block1SHA == block2SHA and block1SHA != block3SHA:
                ...
                self.sock.sendto(f"syncData".encode('utf-8'), (block1Node, 8001))
                ...
                # send remove cmd
                self.sock.sendto(f"removeData".encode('utf-8'), (block1Node, 8001))
                time.sleep(1)
                #send each blockData to block1Node to change block
                try:
                    os.system("rm -f /home/share/*.txt")
                except:
                    pass
                # copy all blockData to block1Node
                for i in range(1, currentBlock+1):
                    os.system(f"cp {i}.txt /home/share/")
                    print(f"cp {i}.txt /home/share/")
                self.sock.sendto(f"changeBlockData  {currentBlock}".encode('utf-8'), (block1Node, 8001))
                print("All blockData sent")
...
if __name__ == '__main__':
    port = 8001 #本節點的port 
    peers = [('172.17.0.20', 8001), ('172.17.0.19', 8001)]  #跟另外二個IP:8001 節點通信
